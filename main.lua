...
function mod:postNpcRender(npc)
    ...
    local newState = updateAnimaSolaState(npc)
    local isRegularDashInProgress = npcData[DASH_ATTACKING_FRAME] ~=0 and npcData[DASH_ATTACKING_FRAME] + ESAU_REGULAR_DASH_FRAME_COUNT >= currentFrameCount
    local isAnimaSolaDashInProgress = npcData[ANIMA_ATTACKING_FRAME] ~=0 and npcData[ANIMA_ATTACKING_FRAME] + ESAU_ANIMA_DASH_FRAME_COUNT >= currentFrameCount

    if isRegularDashInProgress
    or isAnimaSolaDashInProgress
    or newState == AnimaSolaState.STATE_APPLIED then
      if newState == AnimaSolaState.STATE_APPLIED and not isAnimaSolaDashInProgress then
          ...
          mod:drawLine(npc.Position, npcData[TARGET_POSITION], false, 0, isAnimaSolaDashInProgress)
      end
  end
end
...
function mod:drawLine(from, to, flicker, frame, isAnimaSolaDashInProgress)
    ...
    if targetSprite:IsLoaded() then
        if flicker then
            ...
            if isAnimaSolaDashInProgress then
